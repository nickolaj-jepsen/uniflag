//! Per-flag paint functions. Each takes a [`Surface`] and is
...
pub fn paint<S: Surface>(s: &mut S, state: &State, frame: u32, flag_age: u32, connected: bool) {
    if !connected {
    ...
    match (state.flag, state.caution) {
        (Flag::Red, _) => paint_red(s, state.wave, frame, flag_age),
        (_, Caution::VirtualSafetyCar) => paint_vsc(s, frame),
        (_, Caution::SafetyCar) => paint_safety_car(s, frame),
        (Flag::Yellow, _) => paint_yellow(s, state.wave, frame),
        (Flag::Blue, _) => paint_blue(s, state.wave, frame),
        (Flag::Green, _) => paint_green(s, state.wave, frame, flag_age),
        (Flag::White, _) => paint_white(s, state.wave, frame),
        (Flag::Black, _) => paint_black_flag(s, frame),
        (Flag::Orange, _) => paint_orange(s, state.wave, frame),
        (Flag::Checkered, _) => paint_checkered(s, frame),
        (Flag::None, _) => match state.session {
            ...
            Session::Racing | Session::Paused => paint_race_idle(s, frame),
            _ => paint_ready(s, frame, flag_age),
        },
    }
    ...
    if !state.sectors.is_empty() && state.flag != Flag::Red {
        paint_sector_band(s, state.sectors, state.wave, frame);
    }
}

fn paint_yellow<S: Surface>(s: &mut S, wave: WaveLevel, frame: u32) {
    match wave {
        WaveLevel::None => {
            // Static: pronounced cloth-wave overlay (≈59 %..100 % brightness).
            paint_with_wave(s, YELLOW, frame, 150, 255);
        }
        WaveLevel::Single => {
            // Whole-panel strobe at 2 Hz.
            let color = if anim::strobe_60(frame, 2) {
                YELLOW
            } else {
                BLACK
            };
...
fn paint_red<S: Surface>(s: &mut S, wave: WaveLevel, frame: u32, flag_age: u32) {
    ...
    if strobe_hz != 0 {
        ...
        let color = if anim::strobe_60(frame, strobe_hz) {
            RED
        } else {
            BLACK
        };
    ...
    paint_with_wave(s, RED, frame, 150, 255);
}

fn paint_blue<S: Surface>(s: &mut S, wave: WaveLevel, frame: u32) {
    ...
    if sweep_step_frames == 0 {
        paint_with_wave(s, BLUE, frame, 150, 255);
        return;
    }
    ...
    s.fill_with(|x, y| {
        ...
        anim::scale_rgb(BLUE, m)
    });
}

fn paint_green<S: Surface>(s: &mut S, wave: WaveLevel, frame: u32, flag_age: u32) {
    ...
    paint_with_wave(s, GREEN, frame, 220, 255);
}

fn paint_white<S: Surface>(s: &mut S, wave: WaveLevel, frame: u32) {
    let strobe_hz = match wave {
        WaveLevel::None => 0,
        WaveLevel::Single => 3,
        WaveLevel::Double => 5,
    ...
    paint_with_wave(s, WHITE, frame, 220, 255);
}

fn paint_orange<S: Surface>(s: &mut S, wave: WaveLevel, frame: u32) {
...
fn paint_checkered<S: Surface>(s: &mut S, frame: u32) {
...
fn paint_race_idle<S: Surface>(s: &mut S, frame: u32) {
    ...
    s.set_pixel(0, 0, (STATIC_M, STATIC_M, STATIC_M));
    s.set_pixel(max_x, 0, (STATIC_M, STATIC_M, STATIC_M));
    s.set_pixel(0, max_y, (STATIC_M, STATIC_M, STATIC_M));
    s.set_pixel(max_x, max_y, (pulse_m, pulse_m, pulse_m));
}

fn paint_ready<S: Surface>(s: &mut S, frame: u32, flag_age: u32) {
...
fn paint_black_flag<S: Surface>(s: &mut S, frame: u32) {
...
fn paint_caution_board<S: Surface>(s: &mut S, frame: u32, glyphs: &[&[u8; 11]], gap: i32) {
    ...
    for (i, glyph) in glyphs.iter().enumerate() {
        ...
        draw_glyph(s, glyph, ox, y_top, WHITE);
    }
}

/// Stamp a 7×11 1-bpp glyph at `(ox, oy)` in `color`. Bit 6 of each row
/// is the leftmost column, bit 0 is column 6.
fn draw_glyph<S: Surface>(s: &mut S, glyph: &[u8; 11], ox: i32, oy: i32, color: Rgb) {
    for (row, &bits) in glyph.iter().enumerate() {
        for col in 0..GLYPH_W {
            if (bits >> (6 - col)) & 1 != 0 {
                s.set_pixel(ox + col, oy + row as i32, color);
            }
        }
    }
}

fn paint_sector_band<S: Surface>(s: &mut S, mask: SectorMask, wave: WaveLevel, frame: u32) {
...
fn paint_with_wave<S: Surface>(s: &mut S, base: Rgb, frame: u32, lo_mult: u8, hi_mult: u8) {
    s.fill_with(|x, y| {
        let m = anim::wave_mult(x, y, frame, lo_mult, hi_mult);
        anim::scale_rgb(base, m)
    });
}
