...
1. **`runtime::run`** (`runtime.rs`) — owns the `Display`. Select loop: streamed Frame → blit + `present().await`; Brightness → applied on next blit (never to local screens — they always paint full-brightness so a stale sleep value can't hide them); long press → toggle the local test screen; 16 ms tick → advance the local frame counter and repaint whichever local screen is active. No decodable Frame for `CONNECT_TIMEOUT` (1.5 s) → the §7a fallback screen (`screens.rs`), also the boot state.
...
The firmware deliberately does **not** render effects and does not link any render code — the fallback/test screens in `screens.rs` implement `docs/effects-spec.md` §7a bit-exactly with embedded literals. The device stores no settings; brightness persists host-side in the plugin.

### Plugin (`plugin/src/`)

- **Rendering** — the renderer core: `RendererLoop` runs a dedicated thread ticking the **60 fps internal frame counter** all ported animation math assumes, painting via `Effects.Paint` (live game state) or `IdleEffects.PaintConnectedIdle` (stream alive, no game), publishing 3072-byte frames to registered `IFrameSink`s. Sinks *sample* that clock (USB at 30 fps by parity decimation, web overlay at 30 fps, WPF preview at display cadence) — never rebase the counter to a sink rate; that would halve every strobe rate. The thread runs only while ≥ 1 sink is registered. `Rendering/` must stay WPF-free (loadable from plain xunit).
- **Adapters** — telemetry → `RenderState`: `GenericAdapter` (unified `Flag_*` properties, priority + session mapping documented in `docs/simhub-flag-properties.md`) with the iRacing raw-telemetry refiner layered after it (waves, VSC/SC, penalties). One reused snapshot, immutable pipeline — no avoidable allocation on the 60 Hz SimHub data thread.
...
- `testdata/frames/` — the **ported-parity** corpus: 40 raw RGB888 frames + `manifest.json` pinning `(state, frame, flag_age, connected)` tuples; prose spec `docs/effects-spec.md`. **Permanently frozen at M11**: the Rust dumper died with `render/`, so this corpus is unregenerable *by design*. Never rewrite these bytes; the manifest is the ledger (`plugin/tests/GoldenFrameTests.cs` derives one test per entry).
- `testdata/frames-plugin/` — the **C#-authored** corpus (M10 penalty effects), separate from the ported-parity set, with its own regen path (`PluginGoldenDumper`, the `[windows]` `golden-regen` leg, gated on `UNIFLAG_REGEN_PLUGIN_GOLDENS`).
- `testdata/timelines/` — `RenderState` scenario scripts (JSON) replayed through the adapter pipeline by `plugin/tests/TimelineReplayTests.cs`. C#-only (the one non-cross-language subdir) and hand-authored, with no `golden-regen` leg: the two existing files are M7 transcriptions of retired v1 sim scenarios and are frozen (they pin that transcription); new scenarios are added as *new* files in reviewed commits.

`Effects.Paint` is pinned byte-for-byte by the frozen corpus — new rendering behaviour goes in new code paths (`IdleEffects`, penalty painters), never into `Paint`'s existing branches.
...
- v1 history: the ASCII line protocol, the `render/` crate, and the SimHub Custom Serial device profile were retired at M11 — they live in git history only. Old comments citing `render/src/*.rs` paths refer to that history (`docs/effects-spec.md` explains the convention).
