...
- `testdata/frames-grammar/` — the Grammar renderer's conformance corpus (`docs/flag-grammar.md` §11): **script-based** scenarios — a state history replayed through the compositor + envelope from frame 0, with one sampled frame compared byte-exactly (no tolerance). Regenerable via the `[windows]` `golden-regen` leg (`GrammarGoldenDumper`, gated on `UNIFLAG_REGEN_GRAMMAR_GOLDENS`); the ledger is `plugin/tests/GrammarGoldenScenarios.cs` (unique, append-only names; manifest/ledger lockstep is test-enforced, so any table change forces a deliberate regen commit).
...
The Grammar painters are pinned byte-for-byte by `testdata/frames-grammar/` — visual changes regenerate that corpus in deliberate, reviewed commits, never as a test side effect. (The v1 ported-parity corpus `testdata/frames/` and the M10 `testdata/frames-plugin/` corpus were retired with the legacy painters at the flag-grammar cutover; git history keeps the bytes.)
...
- `testdata/**` is immutable outside `just golden-regen`: never edit fixtures by hand, never let a test write them (regen paths are env-var-gated). The one sanctioned hand-authored path is `testdata/timelines/` (see Golden fixtures).
