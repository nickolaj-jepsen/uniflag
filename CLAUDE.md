...
- `cli/` — `uniflag-cli`, host-side test-pattern streamer and protocol diagnostic (stream/doctor modes). Its RX path is pinned against `testdata/proto/` by in-module tests in `cli/src/rx.rs`. `doctor` is the implementation behind `just doctor`.
