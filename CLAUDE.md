...
| `just golden-regen` | regenerate `testdata/proto/` — the only regenerable fixtures left. Run it on purpose, not to make a red test go green. |
...
- `testdata/proto/` — byte vectors for every packet (pre- and post-COBS) plus negative vectors; prose spec `docs/protocol.md`. Regenerable (`golden-regen` proto leg) — but the bytes are what both codecs agree on, so a regen that changes anything is a protocol change.
...
- Retired and in git history only: the v1 ASCII line protocol, the `render/` Rust crate, the SimHub Custom Serial device profile, the legacy C# painters, and the `testdata/frames*` corpora. A comment citing a `render/src/*.rs` path is pointing at that history. `docs/flag-grammar.md` is the live renderer spec.
