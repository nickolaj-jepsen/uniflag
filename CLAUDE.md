...
- `cli/` — `uniflag-cli`, host-side test-pattern streamer and protocol diagnostic (stream/emit/doctor modes). Conformance tests pin its emitted bytes against `testdata/proto/`. `doctor` is the implementation behind `just doctor`.
- `plugin/` — the SimHub plugin (C#), in five projects under `plugin/UniflagPlugin.sln`:
  - `plugin/core/` — **`Uniflag.Core`** (netstandard2.0): **everything that does not need SimHub, WPF or a Windows API** — the renderer (`Rendering/`, incl. `Rendering/Grammar/`), the wire codec (`Protocol/`), the device connection manager (`Device/`), the overlay web server (`Web/`), the telemetry adapters (`Adapters/`) and the debug `StateCycler`. That constraint is the whole point — see below.
  ...
  **The core/simhub boundary is drawn where the platform actually is.** Each net48 file is there because it names a type that does not exist off Windows; each sits behind an interface declared in Core (`IGameAdapter`, `ISerialConnection`, `IPortEnumerator`, `IFrameSink`). When adding code, default to `core/` — put it in `simhub/` only if it cannot compile without `GameReaderCommon`, `System.IO.Ports` or the registry. Same rule for tests.
...
- **Adapters** (`plugin/core/Adapters/`, except `GameDataExtractor`) — telemetry → `SignalState`: `GenericAdapter` (unified `Flag_*` properties, priority/tier + session mapping documented in `docs/simhub-flag-properties.md`) with the iRacing raw-telemetry refiner layered after it (tiers, SC regime, penalties, start sequence, countdown notices). One reused snapshot, immutable pipeline — no avoidable allocation on the 60 Hz SimHub data thread. `GameDataExtractor` (in `plugin/simhub/`) is the only place SimHub types are touched: it copies `GameData` into the plain `TelemetrySnapshot` everything else works from.
...
- **Web** (`plugin/core/Web/`) — `OverlayWebServer`: localhost-only (`127.0.0.1:8972`), hand-rolled HTTP + WebSocket, newest-frame-wins per client. Contracts in `docs/web-overlay.md`.
...
Both the Rust and C# suites consume the same files; **neither side ever generates its own fixtures**; regeneration only via `just golden-regen`, and deliberately — a regen is a decision, not a fix for a failing test. `.gitattributes` marks `testdata/**` `-text`.

- `testdata/proto/` — byte vectors for every packet (pre- and post-COBS) plus negative vectors; prose spec `docs/protocol.md`, per-vector notes in the **hand-written** `testdata/proto/README.md` (as is `.gitattributes`). Only the vectors are regenerable (`golden-regen` proto leg), and the bytes are what both codecs agree on, so a regen that changes anything is a protocol change.
- `testdata/timelines/` — `SignalState` scenario scripts (JSON, format 2) replayed through the adapter pipeline by `plugin/tests-core/TimelineReplayTests.cs`. C#-only (the one non-cross-language subdir) and hand-authored, with no `golden-regen` leg; schema revisions and new scenarios happen in deliberate reviewed commits.

**The renderer is deliberately not pinned to bytes.** `testdata/frames-grammar/` was retired (`docs/flag-grammar.md` §11): while the visual design is still moving, a 41-binary corpus turned every tweak into a mass regeneration whose diff read `Binary files differ`. The painters are covered instead by the `GrammarSmokeTests` whole-scenario replay (churn-free invariants), the per-painter unit tests, and *visual* review via `just frames-sheet`. The scenario catalogue those all share lives in `plugin/core/Rendering/Grammar/ScenarioCatalogue.cs` and is also what `StateCycler` tours in the settings tab — one curated list, three consumers. Don't reintroduce a frame corpus without a reason — the wire protocol is a byte contract, the picture isn't. (Earlier corpora `testdata/frames/` and `testdata/frames-plugin/` went the same way at the flag-grammar cutover; git history keeps all the bytes.)
...
- Don't touch `testdata/**` outside `just golden-regen`: never edit fixtures by hand, never let a test write them (regen paths are env-var-gated). The one sanctioned hand-authored path is `testdata/timelines/` (see Golden fixtures). `golden-regen` now covers `testdata/proto/` only.
...
- Retired and in git history only: the v1 ASCII line protocol, the `render/` Rust crate, the SimHub Custom Serial device profile, the legacy C# painters, the `testdata/frames*` corpora, `testdata/proto/manifest.json` and its generator, the CLI's `loopback` and `view` modes, and the hand-rolled `MiniJson` test parser. A comment citing a `render/src/*.rs` path is pointing at that history. `docs/flag-grammar.md` is the live renderer spec.
